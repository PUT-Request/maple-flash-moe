...
def setup_arg_parser():
    ...
    parser.add_argument(
        "--kv-bits",
        type=int,
        help="Number of bits for K (keys) cache quantization. Defaults to no quantization.",
        default=None,
    )
    parser.add_argument(
        "--kv-v-bits",
        type=int,
        help="Number of bits for V (values) KV cache quantization. Defaults to --kv-bits "
        "when --kv-bits is set, otherwise no quantization. Use e.g. --kv-bits 8 --kv-v-bits 4 "
        "to quantize keys to 8-bit and values to 4-bit.",
        default=None,
    )
    parser.add_argument(
        "--flash-moe",
        action="store_true",
        help="Maple only: keep the full checkpoint file-backed and materialize only a small "
        "activated expert subset per MoE layer into DRAM (AFM 3 Core Advanced style).",
    )
    parser.add_argument(
        "--active-experts",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="flash-moe: always-resident shared experts within the activated set.",
    )
    parser.add_argument(
        "--reselect-every",
        type=int,
...
def maybe_quantize_kv_cache(
    prompt_cache, quantized_kv_start, kv_group_size, kv_bits, kv_v_bits=None
):
    if kv_bits is None:
        return
    for e, c in enumerate(prompt_cache):
        if hasattr(c, "to_quantized") and getattr(c, "offset", 0) >= quantized_kv_start:
            try:
                prompt_cache[e] = c.to_quantized(
                    group_size=kv_group_size, bits=kv_bits, v_bits=kv_v_bits
                )
            except NotImplementedError:
                # Rotating / sliding-window caches don't support quantization
                # in this build; leave them in fp16.
                continue
...
def main():
    ...
    if args.flash_moe:
        from mlx_lm.models.maple import prepare_flash_moe

        prepare_flash_moe(
            model,
            active=args.active_experts,
    ...
    response = generate(
        ...
        kv_v_bits=args.kv_v_bits if args.kv_v_bits is not None else args.kv_bits,
